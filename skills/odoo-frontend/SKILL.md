---
name: odoo-frontend
description: "Use when creating, editing, debugging, or reviewing Odoo 19 frontend or web-client code in JavaScript, XML, QWeb, or frontend addon assets: `@odoo-module`, `@web/...` imports, `web.assets_backend`, services, registries, hooks, client actions, patches, generic components, view architecture, built-in or custom views, `js_class`, field widgets, view widgets, arch XML, modifiers, `options=`, or XPath inheritance. Triggers include `useService`, `patch`, `registry.category(\"views\"|\"fields\"|\"view_widgets\")`, `ArchParser`, `Controller`/`Renderer`/`Model`, `standardFieldProps`, `standardWidgetProps`, `kanban`, `list`, `form`, `graph`, `pivot`, `calendar`, `search`, `xpath`, and `field widget=`. Pair with `owl` for Owl internals and `odoo-19-javascript-testing` for Hoot, web test helpers, and mock-server work. Do not use for broader backend ORM or server-side business logic."
metadata:
  author: Philippe L'ATTENTION
  version: "2026.4.22"
  source: Generated from https://github.com/odoo/documentation and merged with source-verified view references from https://github.com/odoo/odoo, scripts located at https://github.com/phildl/skills
---

> The skill is based on Odoo 19.0 documentation plus source-verified view references, merged at 2026-04-22.
...
| Asset bundles, manifest operations, lazy asset loading, `@odoo-module`, aliases, import rules | [core-assets-and-modules](references/core-assets-and-modules.md) |
| `useService`, `orm` vs `rpc`, notifications, router, effect, title, user service | [core-services](references/core-services.md) |
| Registry categories, sequence ordering, systray, `main_components`, action registry | [core-registries-and-extension-points](references/core-registries-and-extension-points.md) |
